import { A, useParams, useNavigate } from '@solidjs/router';
import { Title, Meta } from '@solidjs/meta';
import { createSignal, For, onMount } from 'solid-js';
import { SolidMarkdown } from 'solid-markdown';

import { getRelativeTime } from '~/utils/relativeTime.ts';
import { Layout } from '~/components/Layout.tsx';
import { Header } from '~/components/Header.tsx';
import { Section } from '~/components/Section.tsx';
import { getPostContent, getReadingTime, POSTS, TAGS } from '~/utils/posts.ts';

export default () => {
	const [relativeTime, setRelativeTime] = createSignal('0 days ago');

	const params = useParams();
	const navigate = useNavigate();
	const post = POSTS.find?.(x => x.slug === params.slug)!;
	if (!post) {
		navigate('/post_not_found', { replace: true });
		return;
	}
	const postTime = post.timestamp;
	onMount(() => {
		setRelativeTime(getRelativeTime(postTime));
	});

	const stats = getReadingTime(params.slug);
	const tagsArr = post.tags.map(x => ({
		slug: x,
		name: TAGS.find(y => y.slug === x)!.name,
	}));
	const postContent = getPostContent(params.slug);

	return (
		<Layout>
			<Title>{post.title} / Ebrahim Haghshenas</Title>
			<Meta name='description' content={post.subtitle} />
			<Header isResume={false} />
			<Section comp='article' cnArg='min-h-full flex flex-col gap-4'>
				<div class='pt-4 px-4 md:pt-8 md:px-8 gap-4 shrink-0 flex flex-col'>
					<div class='text-[#64748b] dark:text-[#94a3b8] text-[0.75rem] leading-[1.375] tracking-normal font-light gap-2 shrink-0 flex flex-wrap justify-start'>
						<div>
							<time datetime={(new Date(postTime)).toJSON()}>
								{relativeTime()}
							</time>
							<span> · </span>
							{stats.minutes?.toFixed()} min read
						</div>
						<span>—</span>
						<ul class='gap-2 shrink-0 flex list-none'>
							<For each={tagsArr}>{tag =>
								<li>
									<A class='text-[#1d4ed8] dark:text-[#60a5fa] font-medium no-underline hover:underline' href={`/tags/${tag.slug}`}>{tag.name}</A>
								</li>
							}</For>
						</ul>
					</div>
					<div class='gap-4 shrink-0 flex flex-col'>
						<h1 class='text-[1.5rem] font-normal text-[#1e293b] dark:text-white shrink-0'>
							{post.title}
						</h1>
						<h2 class='leading-[1.375rem] font-light text-[1.125rem] text-[#1e293b] dark:text-white shrink-0'>
							{post.subtitle}
						</h2>
					</div>
				</div>
				<div class='px-4 pb-4 md:px-8 md:pb-8 gap-4 md:gap-8 shrink-0 flex flex-col'>
					<div class='max-x-[80ch] text-left leading-[1.75] shrink-0'>
						<SolidMarkdown children={postContent} />
					</div>
				</div>
			</Section>
		</Layout>
	);
};
